  ...
  # while linenum < whole_pair:    
   ...
   for s in range(n):
      ...
      # create hdf5 file with data_1 data_2 5-D tensor with dimension (train_pair_num,16,3,224,224)
       #i = 0 
        #pdb.set_trace()
        with  h5py.File('testing_pair1/testing_pair'+str(s)+'.h5','w') as f1_pair:
		...
		f1_pair.create_dataset('label',(train_pair,1),dtype="f8")
		f1_pair.create_dataset('sim',(train_pair,1),dtype="f8") 
        i = 0
...
#	     with open("ucf_siamiese_2.txt") as txt_file2:
		...
		# read video from line info
			  ...
			  f1_pair['data_1'][i,:,frame_num,:,:] = np.swapaxes(cv2.resize(video_1.read()[1],(112,112)),0,2)
			 # pdb.set_trace()
			  f1_pair['data_2'][i,:,frame_num,:,:] = np.swapaxes(cv2.resize(video_2.read()[1],(112,112)),0,2)
		      f1_pair['label'][i]  = int(re.split(' ',line_info_1)[2])
		      #pdb.set_trace() 
		      f1_pair['sim'][i] = int(re.split(' ',line_info_2)[2])
			  #if np.array(f1_pair['data_1'][i,:,frame_num,:,:]).sum() == 0 or np.array(f1_pair['data_2'][i,:,frame_num,:,:]).sum() == 0:
		      print f1_pair['sim'][i][0]
		      i = i+1
		      linenum = linenum + 1
		      print i,linenum,m 
		      if i >= train_pair:
